...
- main.cpp がコンテストで使うコードの雛形である。ここで include している template/template.hpp には `using namespace std;` や rep マクロなどの競技プログラミング特有のイディオムがまとめられている。ライブラリの他の部分はこのファイルを include せず、C++ 的にある程度きれいな書き方で実装されている。
